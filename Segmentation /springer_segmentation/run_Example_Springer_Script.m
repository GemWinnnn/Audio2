...
folder = '../Denoised/normal/';
files = dir(fullfile(folder, '*.wav'));

for k = 1:length(files)
    filename = fullfile(folder, files(k).name);
    [PCG, Fs] = audioread(filename);
    % Call the segmentation function here, e.g.:
    [assigned_states, state_loglik, loglik_matrix, features, hmm_params] = runSpringerSegmentationAlgorithm(PCG, Fs);
    % Save or process results as needed
    disp(['Processed: ', files(k).name]);
end
